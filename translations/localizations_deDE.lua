local _thisLocale = "deDE";

if CerniesWonderfulFunctions.ActiveLanguage ~= _thisLocale then
    -- only load this file if the locale matches
    return
end

CerniesWonderfulFunctions.Localizations[_thisLocale] = {
    ["Arathi Basin"] = "Arathibecken",
    ["Warsong Gulch"] = "Kriegshymnenschlucht",
    ["Alterac Valley"] = "Alteractal",
    
    ["Ahn'Qiraj"] = "Ahn'Qiraj",
    ["Ruins of Ahn'Qiraj"] = "Ruinen von Ahn'Qiraj",
    ["Temple of Ahn'Qiraj"] = "Tempel von Ahn'Qiraj",

    ["Red Qiraji Battle Tank"] = "Roter Qiraji-Kampfpanzer",
    ["Blue Qiraji Battle Tank"] = "Blauer Qiraji-Kampfpanzer",
    ["Green Qiraji Battle Tank"] = "Grüner Qiraji-Kampfpanzer",
    ["Black Qiraji Battle Tank"] = "Schwarzer Qiraji-Kampfpanzer",
    ["Yellow Qiraji Battle Tank"] = "Gelber Qiraji-Kampfpanzer",

    ["Shiny Bauble"] = "Glänzender Köder",
    ["Nightcrawlers"] = "Nachtkriecher",
    ["Bright Baubles"] = "Leuchtender Köder",
    ["Flesh Eating Worm"] = "Fleischfressender Wurm",
    ["Aquadynamic Fish Attractor"] = "Aquadynamischer Fischlockstoff",

    ["Wool Bandage"] = "Wollverband",
    ["Silk Bandage"] = "Seidenverband",
    ["Linen Bandage"] = "Leinenverband",
    ["Runecloth Bandage"] = "Runenstoffverband",
    ["Mageweave Bandage"] = "Magiestoffverband",
    ["Heavy Wool Bandage"] = "Schwerer Wollverband",
    ["Heavy Silk Bandage"] = "Schwerer Seidenverband",
    ["Heavy Linen Bandage"] = "Schwerer Leinenverband",
    ["Heavy Mageweave Bandage"] = "Schwerer Magiestoffverband",
    ["Heavy Runecloth Bandage"] = "Schwerer Runenstoffverband",
    ["Arathi Basin Runecloth Bandage"] = "Runenstoffverband des Arathibeckens",
    ["Warsong Gulch Runecloth Bandage"] = "Runenstoffverband der Kriegshymnenschlucht",
    ["Alterac Heavy Runecloth Bandage"] = "Schwerer Runenstoffverband von Alterac",

    ["Healthstone"] = "Gesundheitsstein",
    ["Minor Healthstone"] = "Geringer Gesundheitsstein",
    ["Major Healthstone"] = "Großer Gesundheitsstein",
    ["Lesser Healthstone"] = "Kleiner Gesundheitsstein",
    ["Greater Healthstone"] = "Mächtiger Gesundheitsstein",

    ["Mana Potion"] = "Manatrank",
    ["Wildvine Potion"] = "Wildrankentrank",
    ["Major Mana Potion"] = "Großer Manatrank",
    ["Minor Mana Potion"] = "Geringer Manatrank",
    ["Major Mana Draught"] = "Großer Manazug",
    ["Combat Mana Potion"] = "Kampfmanatrank",
    ["Lesser Mana Potion"] = "Kleiner Manatrank",
    ["Greater Mana Potion"] = "Mächtiger Manatrank",
    ["Superior Mana Potion"] = "Überragender Manatrank",

    ["Healing Potion"] = "Heiltrank",
    ["Major Healing Potion"] = "Großer Heiltrank",
    ["Minor Healing Potion"] = "Geringer Heiltrank",
    ["Major Healing Draught"] = "Großer Heilzug",
    ["Lesser Healing Potion"] = "Kleiner Heiltrank",
    ["Combat Healing Potion"] = "Kampfheiltrank",
    ["Greater Healing Potion"] = "Mächtiger Heiltrank",
    ["Superior Healing Potion"] = "Überragender Heiltrank",

    ["Mana Ruby"] = "Manarubin",
    ["Mana Jade"] = "Manajade",
    ["Mana Agate"] = "Manaachat",
    ["Mana Citrine"] = "Manacitrin",

    ["Find Herbs"]    = "Kräuter finden",
    ["Find Trees"]    = "Bäume finden",
    ["Find Treasure"] = "Schätze finden",
    ["Find Minerals"] = "Mineralien finden",

    ["Track Beasts"]    = "Tiere aufspüren",
    ["Track Hidden"]    = "Verborgene aufspüren",
    ["Track Giants"]    = "Riesen aufspüren",
    ["Track Demons"]    = "Dämonen aufspüren",
    ["Track Undeads"]   = "Untote aufspüren",
    ["Track Humanoids"] = "Humanoide aufspüren",
    ["Track Dragonkin"] = "Drachenverwandte aufspüren",
    ["Track Elementals"] = "Elementare aufspüren",

    ["Sense Undead"] = "Untote spüren",
    ["Sense Demons"] = "Dämonen spüren",
};
